...
const Page = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/kerusakan-2-tak`, {
            method: 'POST',
            headers: {
    ...
    return (
        <div className="grid-cols-1 container mx-auto items-center py-6 overflow-hidden rounded-lg shadow transition hover:shadow-lg">
            <h3 className="font-semibold text-4xl p-4">Input Data SDS Pada Mesin 2 Tak</h3>
            ...
            <form onSubmit={handleSubmit} className="p-2 mx-12 block text-sm font-medium" >
                ...
                />
                <button type="submit" className="  rounded px-4 py-1.5 text-sm text-white hover:text-color-header bg-color-gray font-semibold">Submit</button>
            </form>
